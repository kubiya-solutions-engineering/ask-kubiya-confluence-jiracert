...
variable "CONFLUENCE_CLIENT_CERT" {
  type        = string
  sensitive   = true
  description = "Client certificate for Confluence authentication"
}

variable "CONFLUENCE_CLIENT_KEY" {
  type        = string
  sensitive   = true
  description = "Client key for Confluence authentication"
}

variable "CONFLUENCE_USER_CREDS" {
  type        = string
  sensitive   = true
  description = "Confluence username and password for authentication"
}

# Optional client certificate authentication
resource "kubiya_secret" "confluence_client_cert" {
  name        = "CONFLUENCE_CLIENT_CERT"
  value       = var.CONFLUENCE_CLIENT_CERT
  description = "Client certificate for Confluence authentication"
}

resource "kubiya_secret" "confluence_client_key" {
  name        = "CONFLUENCE_CLIENT_KEY"
  value       = var.CONFLUENCE_CLIENT_KEY
  description = "Client key for Confluence authentication"
}

resource "kubiya_secret" "confluence_user_creds" {
  name        = "CONFLUENCE_USER_CREDS"
  value       = var.CONFLUENCE_USER_CREDS
  description = "Confluence username and password for authentication"
}
...
data "external" "confluence_content" {
  ...
  query = {
    ...
    CONFLUENCE_USER_CREDS = var.CONFLUENCE_USER_CREDS
    CONFLUENCE_CLIENT_CERT = var.CONFLUENCE_CLIENT_CERT
    CONFLUENCE_CLIENT_KEY = var.CONFLUENCE_CLIENT_KEY
